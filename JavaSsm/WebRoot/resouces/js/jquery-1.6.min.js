...
(function(a, b) {
	...
	function cq() {
		setTimeout(cr, 0);return co = f.now()
	}
	function cg() {
		try {
		...
		f = function() {
			...
			e.fn = e.prototype = {
				...
				pushStack : function(a, b, c) {
					...
					e.isArray(a) ? C.apply(d, a) : e.merge(d, a), d.prevObject = this, d.context = this.context, b === "find" ? d.selector = this.selector + (this.selector ? " " : "") + c : b && (d.selector = this.selector + "." + b + "(" + c + ")");return d
				},
				each : function(a, b) {
				...
				parseXML : function(b, c, d) {
					a.DOMParser ? (d = new DOMParser, c = d.parseFromString(b, "text/xml")) : (c = new ActiveXObject("Microsoft.XMLDOM"), c.async = "false", c.loadXML(b)), d = c.documentElement, (!d || !d.nodeName || d.nodeName === "parsererror") && e.error("Invalid XML: " + b);return c
				},
				noop : function() {},
				...
				sub : function() {
					...
					e.extend(!0, a, this), a.superclass = this, a.fn = a.prototype = this(), a.fn.constructor = a, a.sub = this.sub, a.fn.init = function(c, d) {
						d && d instanceof e && !(d instanceof a) && (d = a(d));return e.fn.init.call(this, c, d, b)
	...
	f.extend({
		...
		}, g.checked = !0, i.noCloneChecked = g.cloneNode(!0).checked, e.disabled = !0, i.optDisabled = !f.disabled;try {
			delete a.test
		} catch (r) {
			i.deleteExpando = !1
		...
		if (a.attachEvent)
			for (p in {submit : 1, change : 1, focusin : 1}) o = "on" + p, q = o in a, q || (a.setAttribute(o, "return;"), q = typeof a[o] == "function"), i[p + "Bubbles"] = q;
		return i
	...
	f.extend({
		...
		removeData : function(b, c, d) {
			if (!!f.acceptData(b)) {
				...
				var k = h[i][e];
				f.support.deleteExpando || h != a ?
					delete h[i]
					: h[i] = null, k ? (h[i] = {}, g || (h[i].toJSON = f.noop), h[i][e] = k) : g && (f.support.deleteExpando ?
					delete b[f.expando]
		...
		queue : function(a, c, d) {
			if (a) {
				...
				d && (!e || f.isArray(d) ? e = f.data(a, c, f.makeArray(d), !0) : e.push(d));return e || []
			}
		},
		...
		promise : function(a, c) {
			...
			var d = f.Deferred(),
				...
				k = a + "mark";
			while (g--)
			if (tmp = f.data(e[g], i, b, !0) || (f.data(e[g], j, b, !0) || f.data(e[g], k, b, !0)) && f.data(e[g], i, f._Deferred(), !0)) h++, tmp.done(l);
			l();return d.promise()
		}
	});
	var n = /[\n\t\r]/g,
	...
	f.fn.extend({
		...
		attr : function(a, c, d, e) {
			...
			if (d !== b) {
				...
				d === !0 && !t.test(c) && (d = c), a.setAttribute(c, "" + d);return d
			}
			if (i && "get" in i && j) return i.get(a, c);
			h = a.getAttribute(c);return h === null ? b : h
		},
			...
			type : {
				set : function(a, b) {
					if (q.test(a.nodeName) && a.parentNode) f.error("type property can't be changed");
						...
						a.setAttribute("type", b), c && (a.value = c);return b
					}
				}
			},
			tabIndex : {
		...
		prop : function(a, c, d) {
			...
			var g,
				h,
				i = e !== 1 || !f.isXMLDoc(a);
			c = i && f.propFix[c] || c, h = f.propHooks[c];return d !== b ? h && "set" in h && (g = h.set(a, d, c)) !== b ? g : a[c] = d : h && "get" in h && (g = h.get(a, c)) !== b ? g : a[c]
		},
		propHooks : {}
		...
		get : function(a, c) {
			...
			d = a.getAttributeNode(c);return d && d.specified ? d.nodeValue : b
		},
		set : function(a, b, c) {
			var d = a.getAttributeNode(c);
			if (d) {
				d.nodeValue = b;return b
			}
		}
			...
			if (a.nodeType !== 3 && a.nodeType !== 8) {
				...
				while (l = c[m++]) {
					h = g ? f.extend({}, g) : {
						handler : d,
						data : e
					}, l.indexOf(".") > -1 ? (n = l.split("."), l = n.shift(), h.namespace = n.slice(0).sort().join(".")) : (n = [], h.namespace = ""), h.type = l, h.guid || (h.guid = d.guid);var o = j[l],
						p = f.event.special[l] || {};
					if (!o) {
						o = j[l] = [];
		...
		remove : function(a, c, d, e) {
			if (a.nodeType !== 3 && a.nodeType !== 8) {
				...
				while (h = c[k++]) {
					...
					if (p.length === 0 || e != null && p.length === 1) (!o.teardown || o.teardown.call(a, m) === !1) && f.removeEvent(a, h, s.handle), g = null,
						delete t[h]
				}
	...
	if (!f.support.changeBubbles) {
		...
		f.event.special.change = {
			...
			teardown : function(a) {
				f.event.remove(this, ".specialChange");return y.test(this.nodeName)
			}
	...
	f.support.focusinBubbles || f.each({
		...
		f.fn[c] = function(a, d, e) {
			...
			c === "one" ? (g = function(a) {
				f(this).unbind(a, g);return e.apply(this, arguments)
			}, g.guid = e.guid || f.guid++) : g = e;
			if (a === "unload" && c !== "one") this.one(a, d, e);else
		...
		toggle : function(a) {
			var b = arguments,
				...
				e = function(c) {
					...
					f.data(this, "lastToggle" + a.guid, e + 1), c.preventDefault();return b[e].apply(this, arguments) || !1
				};
			e.guid = c;
	...
	f.each([ "live", "die" ], function(a, c) {
		...
		var k = function(b, d, f, g) {
			...
			o && (k(o, h, f, g), k.uniqueSort(f));return f
		};
		k.uniqueSort = function(a) {
			...
			d || (d = typeof b.getElementsByTagName != "undefined" ? b.getElementsByTagName("*") : []);return {
				set : d,
				expr : a
			}
			...
			while (a && c.length) {
				...
				if (a === h)
					if (g == null) k.error(a);else break;
				h = a
			}
			return j
		}, k.error = function(a) {
		...
		var l = k.selectors = {
				...
				preFilter : {
					...
					ATTR : function(a, b, c, d, e, f) {
						...
						!f && l.attrMap[g] && (a[1] = l.attrMap[g]), a[4] = (a[4] || a[5] || "").replace(i, ""), a[2] === "~=" && (a[4] = " " + a[4] + " ");return a
					},
					...
					if (!g && !k.isXML(e)) {
						...
						if (e.nodeType === 9) {
							...
							var m = e,
								...
								q = e.parentNode,
								r = /^\s*[+~]/.test(b);
							n ? o = o.replace(/'/g, "\\$&") : e.setAttribute("id", o), r && q && (e = e.parentNode);try {
	...
	f.fn.extend({
		...
		f.fn[a] = function(c, d) {
			var e = f.map(this, b, c),
				g = S.call(arguments);
			O.test(a) || (d = c), d && typeof d == "string" && (e = f.filter(d, e)), e = this.length > 1 && !U[a] ? f.unique(e) : e, (this.length > 1 || Q.test(d)) && P.test(a) && (e = e.reverse());return this.pushStack(e, a, g.join(","))
		}
	}), f.extend({
	...
	be.optgroup = be.option, be.tbody = be.tfoot = be.colgroup = be.caption = be.thead, be.th = be.td, f.support.htmlSerialize || (be._default = [ 1, "div<div>", "</div>" ]), f.fn.extend({
		...
		after : function() {
			...
			if (arguments.length) {
				...
				a.push.apply(a, f(arguments[0]).toArray());return a
			}
		},
		remove : function(a, b) {
		...
		a.length === 1 && typeof a[0] == "string" && a[0].length < 512 && i === c && a[0].charAt(0) === "<" && !bb.test(a[0]) && (f.support.checkClone || !bc.test(a[0])) && (g = !0, h = f.fragments[a[0]], h && h !== 1 && (e = h)), e || (e = i.createDocumentFragment(), f.clean(a, i, e, d)), g && (f.fragments[a[0]] = h ? e : 1);return {
			fragment : e,
			cacheable : g
		}
	}, f.fragments = {}, f.each({
		...
		clean : function(a, b, d, e) {
			...
			if (d) {
				...
				for (i = 0; h[i]; i++)
					if (e && f.nodeName(h[i], "script") && (!h[i].type || h[i].type.toLowerCase() === "text/javascript")) e.push(h[i].parentNode ? h[i].parentNode.removeChild(h[i]) : h[i]);
					else {
						if (h[i].nodeType === 1) {
	...
	f.fn.css = function(a, c) {
		...
		f.cssHooks[b] = {
			get : function(a, c, d) {
				...
				if (c) {
					...
					if (e < 0 || e == null) {
						e = a.style[b];return e === "" || e === "auto" ? "0px" : e
					}
		...
		var c,
			...
			f = a.style;
		!bq.test(d) && br.test(d) && (c = f.left, e && (a.runtimeStyle.left = a.currentStyle.left), f.left = b === "fontSize" ? "1em" : d || 0, d = f.pixelLeft + "px", f.left = c, e && (a.runtimeStyle.left = e));return d === "" ? "auto" : d
	...
	try {
		...
		f[c] = function(a, d, e, g) {
			f.isFunction(d) && (g = g || e, e = d, d = b);return f.ajax({
				type : c,
				url : a,
				data : d,
				success : e,
					...
					c.mimeType && h.overrideMimeType && h.overrideMimeType(c.mimeType), !c.crossDomain && !e["X-Requested-With"] && (e["X-Requested-With"] = "XMLHttpRequest");try {
						...
						try {
							if (d && (e || h.readyState === 4)) {
								d = b, i && (h.onreadystatechange = f.noop, cc &&
								delete ce[i]
								);
	...
	var ch = {},
		...
		cl = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i,
		cm,
		cn = [ [ "height", "marginTop", "marginBottom", "paddingTop", "paddingBottom" ], [ "width", "marginLeft", "marginRight", "paddingLeft", "paddingRight" ], [ "opacity" ] ],
		co,
	...
	f.fn.extend({
		...
		toggle : function(a, b, c) {
			...
			f.isFunction(a) && f.isFunction(b) ? this._toggle.apply(this, arguments) : a == null || d ? this.each(function() {
				...
				f(this)[b ? "show" : "hide"]()
			}) : this.animate(cs("toggle", 3), a, b, c);return this
		},
	...
	"getBoundingClientRect" in c.documentElement ? f.fn.offset = function(a) {
		...
		f.offset.supportsFixedPosition && k.position === "fixed" && (l += Math.max(h.scrollTop, i.scrollTop), m += Math.max(h.scrollLeft, i.scrollLeft));return {
			top : l,
			left : m
		}
	}, f.offset = {
		...
		bodyOffset : function(a) {
			var b = a.offsetTop,
				c = a.offsetLeft;
			f.offset.initialize(), f.offset.doesNotIncludeMarginInBodyOffset && (b += parseFloat(f.css(a, "marginTop")) || 0, c += parseFloat(f.css(a, "marginLeft")) || 0);return {
				top : b,
				left : c
			}
		},
		...
		position : function() {
			...
			c.top -= parseFloat(f.css(a, "marginTop")) || 0, c.left -= parseFloat(f.css(a, "marginLeft")) || 0, d.top += parseFloat(f.css(b[0], "borderTopWidth")) || 0, d.left += parseFloat(f.css(b[0], "borderLeftWidth")) || 0;return {
				top : c.top - d.top,
				left : c.left - d.left
			}
		},
		...
		f.fn[d] = function(c) {
			...
			if (c === b) {
				e = this[0];
				if (!e) return null;
				g = cw(e);return g ? "pageXOffset" in g ? g[a ? "pageYOffset" : "pageXOffset"] : f.support.boxModel && g.document.documentElement[d] || g.document.body[d] : e[d]
			}
			return this.each(function() {
