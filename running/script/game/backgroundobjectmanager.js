...
function BackgroundObjectManager(context, level, theme)
{
	...
	var TREES = [];
	for (var t = 1; t <= (theme == "countryside2" ? 5 : 6); t++) {
		TREES.push({chance:1, data:new GameObjectImageData(oiu(theme+"/boom" + t), DEFAULT_BASE_SCALE * (theme == "countryside2" ? 0.75 : 1.5) * 0.8, GO_FLIPSTYLE_RANDOM, 1, 0.4)});
	};
...
//						{chance:1, data:new GameObjectImageData(loadImage("images/pixelkaiju-sidemonster05-right.png"), DEFAULT_BASE_SCALE)}];
	...
	function getBackgroundObject(lane, side)
	{
		...
		if (theme == "countryside2" && Math.random() > 0.2) return null;
...
//				if (side == ROAD_SIDE_LEFT){
					...
					if (theme == "countryside1") families.push({objects:SHROOMS, chance:1});
//				}else{
