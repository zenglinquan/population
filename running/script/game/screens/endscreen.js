...
function EndScreen(context, game)
{
    ...
    this.buttons.push(new Button(context, getLangString("replay").toUpperCase(), bbLeft + (1*bbSpacing), 350, bbBWidth, 70, [buttonImg], BUTTON_LAYOUT_SMALL_BUTTON, function(){
    	game.startLevel(-1);
    	$('.breatheDiv').removeClass('ani_circle');
    }, 0.33));
    this.buttons.push(new Button(context, getLangString("back").toUpperCase(), bbLeft + (2*bbSpacing), 350, bbBWidth, 70, [buttonImg], BUTTON_LAYOUT_SMALL_BUTTON, function(){
    	game.showStartScreen();
    	$('.breatheDiv').removeClass('ani_circle');
    	$('#picture').find('div').css({'opacity':'0'});
    }, 0.33));
...
//  }
    ...
    this.result = {eggs:0, score:0, hits:0,distance:0, succes:false};
};

EndScreen.prototype.setResult = function(eggs, score, hits,distance, succes)
{
	if (typeof succes === "undefined") debugger;
	 this.result.eggs = eggs;
	 this.result.score = score;
	 this.result.hits = hits;
	 this.result.distance=Math.floor(distance);
...
EndScreen.prototype.update = function(timeFactor)
{
	if (this.isVisible){
	    ...
	    strokedText(context, getLangString("hits").replace("$", this.result.hits), 300, 190);
	    strokedText(context, getLangString("coins").replace("$", this.result.eggs), 300, 230);
	    strokedText(context, getLangString("score").replace("$", this.result.score), 300, 270);
	    strokedText(context, getLangString("distance").replace("$", this.result.distance), 300, 310);
