...
//			game.showEndScreen(this.eggs, this.score, this.hits, true, this.getResultString());
			game.showEndScreen(this.eggs, this.score, this.hits,this.currentDistance, true, this.getResultString());
//			console.log("leveljs中showEndScreen打印"+this.currentDistance)
    	}
    };
...
//      console.log("这是leveljs中update打印的路程"+this.currentDistance);

        //Draw/update HUD
        this.hud.update(this.eggs, this.lifes, this.score, this.currentDistance, (this.totalTime - this.currentTime));
        var rate_distance=0;
        var self=this;
        	rate_distance=self.currentDistance/ALLDISTANCE;
		function HighlightCircle(){
//			console.log(rate_distance);
			switchFunction(rate_distance.toFixed(3))
		}
  		function switchFunction(rate_distance){
			switch (rate_distance){
				case "0.050":
				console.log("0.05");
				$('#circle_1').addClass('ani_circle');
				break;
				case "0.300":
				$('#circle_2').addClass('ani_circle');
				break;
				case "0.350":
				$('#circle_3').addClass('ani_circle');
				break;
				case "0.450":
				$('#circle_4').addClass('ani_circle');
				break;
				case "0.500":
				$('#circle_5').addClass('ani_circle');
				break;
				case "0.600":
				$('#circle_6').addClass('ani_circle');
				break;
				case "0.700":
				$('#circle_7').addClass('ani_circle');
				break;
				case "0.850":
				$('#circle_8').addClass('ani_circle');
				break;
				case "0.900":
				$('#circle_9').addClass('ani_circle');
				break;
				default:
				break;
			}	
		...
		HighlightCircle();
		controlTheme();
		function themeChange(themeName,roadColorString){
			theme=themeName;
			roadColor=roadColorString;
	    	self.backgroundMountains = new MovingBackground(context, self, theme);
	    	backgroundImg = loadImage("images/env/" + theme + "/sky.jpg");  //loadImage("images/background" + this.levelId + ".jpg"); // backgrounds[levelId - 1];
    		grassImg = loadImage("images/env/" + theme + "/floor.jpg"); //loadImage("images/grass" + this.levelId + ".png"); //grasses[levelId - 1];
		}
		function controlTheme(){
		   	if (rate_distance >=0.050) {
		   		themeChange("countryside2","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >=0.300) {
		   		themeChange("countryside3","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >=0.350) {
		   		themeChange("countryside4","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >0.450) {
		   		themeChange("countryside5","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >=0.500) {
		   		themeChange("countryside6","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >0.600) {
		   		themeChange("countryside7","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >=0.700) {
		   		themeChange("countryside8","rgba(250,248,112,0.5)");
		    }
		  	if (rate_distance >0.850) {
		   		themeChange("countryside9","rgba(250,248,112,0.5)");
		    }
		  	
		}
		
	/*    if (this.currentDistance >300) {
	    	theme = "snow";
	    	roadColor = "rgba(255,255,255,0.5)";
	    }*/
    };
...
//    
    ...
    this.handlePlayerGhostAnimDone = function(){
    	game.showEndScreen(this.eggs, this.score, this.hits,this.currentDistance, false, this.getResultString());
...
//    };
    ...
    this.preSpawnBackgroundObjects = function(){
    	...
    	function fakeUpdate(){
        	...
        	//Update speed
        	var fakeTimeFactor = 0.6; //6.4;
        	thisRef.uncorrectedSpeed += (0.000010 * fakeTimeFactor); //+ (timePast / 1000000) //0.00005;
